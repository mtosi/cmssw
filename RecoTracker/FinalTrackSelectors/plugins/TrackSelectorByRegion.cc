  ...
  class TrackSelectorByRegion final : public edm::global::EDProducer<> {
   public:
    ...
    explicit TrackSelectorByRegion(const edm::ParameterSet& conf) :
      ...
      inputTrkRegionToken_ = consumes<edm::OwnVector<TrackingRegion> >(conf.getParameter<edm::InputTag>("input"));
      //      }
   ...
   private:
	...
	if ( tracksHandle.isValid() ) {
	  ...
	  if ( regionsHandle.isValid() ) {
	    ...
	    std::vector<GlobalPoint> origin; origin.reserve(n_regions);
		...
		phiMin.push_back( Geom::Phi<float>(etaPhiRegion->phiDirection() - phiMargin.left()) );
		phiMax.push_back( Geom::Phi<float>(etaPhiRegion->phiDirection() + phiMargin.right()) );

		origin.push_back( etaPhiRegion->origin()       );
